@section('dashboard-active', 'bg-gray-100 group') <div class="p-4">
  ...
  <div class="flex flex-col items-center">
    <label for="loadingReportYearSelect" class="text-md font-semibold text-gray-700 dark:text-gray-300 mb-2">Loading Report:</label>
    ...
    <div class="w-full bg-white shadow-md dark:bg-gray-800 rounded-lg p-4 mt-6">
      ...
      <div class="overflow-x-auto rounded-lg border border-gray-200">
        <table class="w-full text-sm text-gray-700 dark:text-gray-300">
          ...
          <tbody id="loadingReportTableBody" class="divide-y divide-gray-200 dark:divide-gray-600 bg-white dark:bg-gray-800">
            <tr>
              <td colspan="2" class="text-center py-2 text-gray-500">Loading...</td>
            </tr>
          </tbody>
  ...
  document.addEventListener("DOMContentLoaded", function() {
    ...
    function fetchTableData() {
      ...
      fetch(`{{ route('api.table') }}?year=${year}&month=${month}`).then(response => response.json()).then(data => {
...
<tr>
    ...
    function fetchProductionData(year) {
      fetch(`{{ route('api.productionreport') }}?year=${year}`).then(response => response.json()).then(data => {
          ...
          productionChart = new Chart(productionCtx, {
            ...
            data: {
              ...
              datasets: Object.keys(productionData).map((line, index) => ({
                ...
                backgroundColor: ["rgba(255, 99, 132, 0.5)", "rgba(54, 162, 235, 0.5)", "rgba(255, 159, 64, 0.5)", "rgba(75, 192, 192, 0.5)", "rgba(153, 102, 255, 0.5)", "rgba(255, 159, 64, 0.5)"][index],
                borderColor: ["rgba(255, 99, 132, 1)", "rgba(54, 162, 235, 1)", "rgba(255, 159, 64, 1)", "rgba(75, 192, 192, 1)", "rgba(153, 102, 255, 1)", "rgba(255, 159, 64, 1)"][index],
                borderWidth: 1
              }))
            },
            options: {
    ...
    function fetchLoadingReportData(year, month) {
      fetch(`{{ route('api.loadingreport') }}?year=${year}&month=${month}`).then(response => response.json()).then(data => {
