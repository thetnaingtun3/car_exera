...
class ChangCanningLineOne extends Component
{
    use WithPagination;
...
//        return Excel::download(new PalletRegistrationExport($this->startDate, $this->endDate), 'filtered_data.xlsx');
    ...
    public function render()
    {
        ...
        $volumes = PalletRegister::distinct()->where('product_type', 'Chang beer')->where('production_line', 'Canning line 1')->pluck('volume');
        $this->count = $pallets->total();

        return view('livewire.line.chang-canning-line-one', compact('pallets', 'productTypes', 'productionLines', 'volumes'));
    }
